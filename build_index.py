...
- 圖檔／音檔：`downloads/ICASSP2027_elw/`（使用者指定來源）
  ...
  - `result_csv/fig/paper/fig1_flow_paper_f4.png` → 論文 Fig. 1（f4 版，Stage 3 標籤 m2m EL→NL）
...
ABSTRACT = (
    ...
    "closer to natural (NL) speech. Self-supervised learning (SSL) models are primarily used as frozen feature "
    "extractors for voice conversion (VC) and are rarely fine-tuned for EL speech, which is extremely challenging "
    "due to the scarcity of patient EL data. We propose EL-WavLM, a WavLM-Large encoder adapted to EL data "
    "through staged fine-tuning (optionally with the addition of a teacher loss), to replace the encoder of the "
    "electrolaryngeal speech transformer network (ETN). The resulting system ETN-elwavlm is then adapted "
    "one-to-one (o2o) for each patient&rsquo;s EL (PEL) speech. Before o2o adaptation, ETN-elwavlm already "
    "achieved a Whisper character error rate (W-CER) of 65.1% on the PEL&ndash;NL pairs; after o2o adaptation, "
    "the best setting (full staged fine-tuning without teacher loss) reached 62.5%, compared to 88.3% for ETN-mel "
    "(mel-spectrogram input) and 82.8% for ETN-wavlm (frozen WavLM features)."
)
...
CSS = """
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* 懸掛縮排：項目符號絕對定位在左側，換行的第二行對齊文字而不是退回符號底下 */
    .dataset-card ul li { padding: 3px 0 3px 1.1em; position: relative; }
    .dataset-card ul li::before { content: '\\00b7'; color: var(--accent); position: absolute; left: 0.2em; }
...
<div class="container">
  ...
  <section>
    ...
    <div class="fig-block">
      ...
      <p class="fig-caption"><b>Fig. 1.</b> (a)&nbsp;Training flow of the ETN baseline model (top) and the proposed
        ...
        many-to-many (m2m) EL&ndash;NL pairs (Stage&nbsp;3), and one-to-one (o2o) adaptation to a PEL&ndash;NL pair
        (Stage&nbsp;4). The difference lies in Stage&nbsp;3: in ETN, the encoder&ndash;decoder is fitted to all EL&ndash;NL
    ...
    <ul class="stage-list">
      ...
      <li><b>Stage 2</b>Self-reconstruction of NL speech on the same corpus with the decoder frozen: the ETN baselines pretrain
    ...
    <div class="dataset-grid">
      ...
      <div class="dataset-card">
        ...
        <ul>
          ...
          <li>Six healthy speakers (4 men and 2 women) recorded simulated EL (SEL) and NL speech; two male
            laryngectomees recorded patient EL (PEL) speech: PEL03, PEL11</li>
          <li>Stage 3 is many-to-many over all 8 EL speakers (6 SEL and 2 PEL) &times; 6 NL speakers, i.e.
            8&nbsp;&times;&nbsp;6 EL&ndash;NL pairs, using only the training portion of each pair</li>
  ...
  <section>
    ...
    <p class="fig-caption">A subjective intelligibility test was also carried out. Twelve listeners (9 men and 3 women)
    ...
    <div class="metrics-block">
      ...
      <table class="metrics-table">
        ...
        <tbody>
          <tr>
            <td>ETN-mel</td>
            <td class="num">2.07 &plusmn; 0.36</td>
          </tr>
          <tr>
            <td>ETN-wavlm</td>
            <td class="num">2.47 &plusmn; 0.38</td>
          </tr>
          <tr class="proposed rowsep">
            <td><b>ETN-elwavlm (proposed)</b></td>
            <td class="num best">3.21 &plusmn; 0.37</td>
          </tr>
          <tr>
            <td>PEL (unprocessed)</td>
            <td class="num">1.23 &plusmn; 0.20</td>
          </tr>
          <tr>
            <td>NL (reference)</td>
  ...
  <!-- Spectrograms + single-sentence t-SNE + audio, one card per sample -->
  <section>
    <div class="section-label">Samples</div>
    <h2 class="section-title">Spectrograms, Single-Sentence t-SNE and Audio Samples</h2>
